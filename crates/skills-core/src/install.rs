...
/// 不阻断安装（避免破坏现有可安装技能）。
fn report_frontmatter_issues(sink: &SharedSink, dest: &Path) {
...
fn standardize_if_agents_dir(sink: &SharedSink, skills_path: &Path, dest: &Path) {
...
pub async fn install_skill_streamed(
    ...
    match try_npx_install(&sink, &normalized_source, skill_name, &target_label).await {
        Ok(()) => {
            ...
            let skills = crate::scan::scan_directory(&skills_path);
            ...
            let (dest_path, verified, files) = match installed {
                Some(s) => {
                    ...
                    let _ = crate::scan::write_manifest(&p, &manifest);
...
fn emit_done(sink: &SharedSink, success: bool, target_label: &str) {
    sink.emit(
...
async fn try_npx_install(
    ...
    let stdout_handle = tokio::spawn(async move {
        ...
        while let Ok(Some(line)) = lines.next_line().await {
            let _ = sink_stdout.emit(InstallOutputEvent::Stdout { data: line });
        }
    });
    ...
    let stderr_handle = tokio::spawn(async move {
        ...
        let mut lines = reader.lines();
        while let Ok(Some(line)) = lines.next_line().await {
            let _ = sink_stderr.emit(InstallOutputEvent::Stderr { data: line });
        }
    });
