...
- **使用统计与趋势** — 操作历史页聚合安装 / 卸载 / 启停数据，近 14 天趋势图 + Top 技能排行
- **收藏导入导出** — 收藏列表可导出为 JSON 文件并在另一台机器导入
- **会话恢复** — 重启后自动还原上次的搜索关键词、筛选与视图状态
- **离线本地安装** — 直接复制本地技能目录完成安装，无需网络
- **缓存一键清理** — 统计并清理 GitHub / 技能 / 翻译缓存，防止磁盘空间失控
- **崩溃日志** — Rust panic 与前端渲染错误自动落盘，便于问题追踪
- **主题定制** — 亮色 / 暗色 / 跟随系统；强调色 6 种预设 + 自定义取色器，改动即时生效
- **主题适配** — 表面色（背景 / 边栏 / 卡片）走 CSS 变量驱动，任意透明度变体在亮 / 暗主题下均正确显示；浅色下强调色自动取同色系深色变体，保证文字与图标对比度
...
在 **设置 → 外观** 中切换 **亮色 / 暗色 / 跟随系统**，并选择强调色（荧光绿、蓝色、紫色、橙色、青色、粉色 6 种预设，或使用取色器自定义任意颜色）。

主题系统采用 **CSS 变量驱动**，全局一致、改动即时生效：

- **表面色**（页面背景 / 边栏 / 卡片）与强调色均以 RGB 三元组存入 CSS 变量，Tailwind 的 `bg-trae-*` 系列颜色统一引用，任意透明度变体（如 `bg-trae-card/30`）在亮 / 暗主题下都自动正确显示，无需逐处适配。
- **浅色自动压暗**：浅色主题下强调色自动生成同色系深色变体，保证按钮、链接、勾选框、趋势图与关系图节点等元素的文字和图标在白底上清晰可读。
- **即时生效**：切换主题或修改强调色时立即重算并应用，无需重启；重启后自动还原上次的主题与强调色。
- 技能关系图、操作趋势图等 SVG 渲染区域同样跟随主题与强调色，避免硬编码颜色在浅色下不可见。
